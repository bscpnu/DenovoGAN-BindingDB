...
class CustomSaver(keras.callbacks.Callback):
    ...
    def on_epoch_end(self, epoch, logs={}):
        if epoch % 20 == 0:  # or save after some epoch, each k-th epoch etc.
            self.encoder.save("model_vae_target/encoder/encoder_{}.h5".format(epoch))
            self.decoder.save("model_vae_target/encoder/decoder_{}.h5".format(epoch))
...
if __name__ == '__main__':
    ...
    saver = CustomSaver(encoder, decoder)
    g_model.fit(ls_target, ls_target, batch_size=50, epochs=200, callbacks=[saver])
